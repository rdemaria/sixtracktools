    ...
    variables = OrderedDict(
        [('title', Variable('', 'START', 'Study title')),
         ('geom',  Variable('GEOM', 'START',
            ...
            ('deco_name6', Variable('', 'DECO',
                                    'Name of defocusing quadrupole families')),
            ...
            ('diff_preda', Variable(1e-38, 'DIFF',
                                    'Precision needed by the DA package')),
            ('diff_nsix',
            ...
            ('aper1', Variable(1000, 'ITER',
                               'Horizontal aperture limit [mm]')),
            ...
            ('ibidu', Variable(1, 'TRAC',
                               'Switch to create or read binary dump of accelerator')),
            ...
            # BLOCKS OF INFORMATION FOR FROM FORT.3 IN ALPHABETICAL ORDER
                ...
                while not currline.startswith('NEXT'):
                    ...
                    for ii in range(0, 12):
                        currline = next(f3)
                        Mvar = Mvar + [myfloat(item)
                                       for item in currline.split()]
                    data = cvar + Mvar
                    self.trom[name] = data
                    currline = next(f3)
